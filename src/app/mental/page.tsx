...
function RatingSelector({ value, onChange, label, color }: { value: number | null; onChange: (v: number) => void; label: string; color: string }) {
  return (
    <div>
      <label className="text-[13px] font-semibold text-gray-400 uppercase tracking-wider mb-2 block">{label}</label>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[28px] font-bold text-gray-50 tracking-tight">Mental Game</h1>
        ...
        <div className="bg-gray-900 rounded-2xl p-4 sm:p-5 mb-6 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            {(logType === 'post_round' || logType === 'pre_round') && (
              <div>
                <label className="text-[13px] font-semibold text-gray-400 uppercase tracking-wider mb-2 block">Linked Round</label>
                <select
                  value={roundId}
          ...
          {showOptional && (
            <div className="space-y-4 border-t border-gray-800/60 pt-4">
              {/* Mental triggers */}
              <div>
                <label className="text-[13px] font-semibold text-gray-400 uppercase tracking-wider mb-2 block">Mental Triggers</label>
              ...
              {/* Positive moments */}
              <div>
                <label className="text-[13px] font-semibold text-gray-400 uppercase tracking-wider mb-2 block">Positive Moments</label>
                <div className="flex flex-wrap gap-1.5">
              ...
              {/* Pre-shot routine */}
              <div>
                <label className="text-[13px] font-semibold text-gray-400 uppercase tracking-wider mb-2 block">Pre-Shot Routine</label>
                <textarea
                  ...
                  {expanded && (
                    <div className="border-t border-gray-800/60 p-4 space-y-3">
                      {l.mental_triggers.length > 0 && (
                        <div>
                          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 px-1">Triggers</span>
                          <div className="flex flex-wrap gap-1 mt-1">
                            {l.mental_triggers.map((t) => (
                              <span key={t} className="text-xs px-2 py-1 rounded-full bg-red-500/10 text-red-400">{t}</span>
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      {l.positive_moments.length > 0 && (
                        <div>
                          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 px-1">Positives</span>
                          <div className="flex flex-wrap gap-1 mt-1">
                            {l.positive_moments.map((p) => (
                              <span key={p} className="text-xs px-2 py-1 rounded-full bg-green-500/10 text-green-400">{p}</span>
                            ))}
                          </div>
                        </div>
                      )}
...
function TrendCard({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="bg-gray-900 rounded-2xl p-4">
      <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 px-1">{label}</div>
